'use strict';
// Testing the user's hypothesis: the Infinite path requires an EXACT
// landing on square 36, which likely makes it structurally rarer than just
// capturing the Soul. easyInfiniteEntry relaxes that to "reaches or passes
// TRACK_LEN, once Body is captured" - still gated on Body capture, just not
// on hitting the number precisely. Tested alone and combined with
// allowLooping (the confirmed draw-rate fix), against the LIVE baseline,
// at the bigger 200-game sample size that gave us a trustworthy read last
// time.
//
//   node nefesh-rule-sweep5.js [gamesPerConfig] [iterations]
...
function scoreConfig(label, ruleOpts, gamesCount){
  sim.setRuleFlags({...sim.LIVE_RULES, allowLooping:false, easyInfiniteEntry:false, ...ruleOpts});
...
const CONFIGS = [
  ...
  {label:'easyInfiniteEntry alone', rules:{easyInfiniteEntry:true}},
  {label:'allowLooping alone', rules:{allowLooping:true}},
  {label:'easyInfiniteEntry + allowLooping', rules:{easyInfiniteEntry:true, allowLooping:true}},
];

console.log(`=== Sweep 5 (easyInfiniteEntry hypothesis test): ${CONFIGS.length} configs, ${GAMES} games each, ${ITERATIONS} iterations/move ===`);
const t0 = Date.now();
